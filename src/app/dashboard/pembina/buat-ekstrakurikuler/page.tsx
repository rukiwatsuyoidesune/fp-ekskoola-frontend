...
export default function CreateExtracurricularPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/ekstra`, {
        method: "POST",
        headers: {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50">
      <header className="bg-white/80 backdrop-blur-sm border-b sticky top-0 z-50">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <Button type="submit" form="ekstraForm" className="bg-gradient-to-r from-blue-700 to-blue-800 hover:from-blue-800 hover:to-blue-900">
      ...
      <div className="container mx-auto px-4 py-8">
        <form id="ekstraForm" onSubmit={handleSubmit} className="grid lg:grid-cols-3 gap-8">
            ...
            <Card className="border-0 shadow-lg">
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  ...
                  <Input id="maxMembers" type="number" value={formData.maxMembers} onChange={(e) => setFormData({ ...formData, maxMembers: e.target.value })} min="1" />
                </div>
              </CardContent>
            </Card>
                ...
                {schedules.map((schedule) => (
                  <div key={schedule.id} className="p-4 border rounded-lg bg-gray-50">
                    <div className="grid md:grid-cols-3 gap-4">
                      <div>
                        <Label>Hari</Label>
                        <Select value={schedule.day} onValueChange={(value) => updateSchedule(schedule.id, "day", value)}>
                          <SelectTrigger>
                      ...
                      <div>
                        ...
                        <Input type="time" value={schedule.startTime} onChange={(e) => updateSchedule(schedule.id, "startTime", e.target.value)} />
                      </div>
                      <div>
                        <Label>Waktu Selesai</Label>
                        <Input type="time" value={schedule.endTime} onChange={(e) => updateSchedule(schedule.id, "endTime", e.target.value)} />
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <CardContent className="space-y-4">
                <div className="grid md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input id="registrationStart" type="date" value={formData.registrationStart} onChange={(e) => setFormData({ ...formData, registrationStart: e.target.value })} />
                  </div>
                  <div>
                    <Label htmlFor="registrationEnd">Tanggal Berakhir</Label>
                    <Input id="registrationEnd" type="date" value={formData.registrationEnd} onChange={(e) => setFormData({ ...formData, registrationEnd: e.target.value })} />
                  </div>
                </div>
              </CardContent>
            </Card>
